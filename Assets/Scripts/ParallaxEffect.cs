...
public class EffetParallaxe : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (cameraCible == null) cameraCible = Camera.main.transform;

        if (cameraCible != null) positionCameraInitiale = cameraCible.position;
    }

    private void LateUpdate()
    {
        if (cameraCible == null && Camera.main != null) return;
